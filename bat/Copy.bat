...
if %NUM%==1 (
  ...
  echo 1~3以外が入力されました。１～３の数字を入力してください
  goto CHOICE
)

:SUB
set SUB=
set /P SUB="サブカテゴリを入力　例：guardian など　"
if exist %IMG%\%SUB% goto NEXT
if not exist %IMG%\%SUB% echo "そのディレクトリはありませんよ"
goto SUB
...
echo ↑のファイルをコピーします
echo サブカテゴリが空白の場合、
echo コピーすると重くなるので、↑を見てサブカテゴリを入力してください
echo ----------------------------------------------------
pause

rem for /r %%I in (*) do xcopy /S /D:%MONTH%-%DAY%-2018 %%I %DML%\%SUB%
xcopy /F /S /Y /I /D:%MONTH%-%DAY%-2018 /D %IMG%\%SUB% %DML%\%SUB%
rem /F コピーしたファイル名を表示　/S サブディレクトリもコピー　/Y 上書き確認しない　/D:xxx 日付より最新のファイルのみ
echo ----------------------------------------------------
echo コピー完了
echo ----------------------------------------------------
