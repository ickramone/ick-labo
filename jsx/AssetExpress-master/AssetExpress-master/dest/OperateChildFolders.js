...
var OperateChildFolders, ocf,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;

OperateChildFolders = (function(superClass) {
  extend(OperateChildFolders, superClass);

  OperateChildFolders.prototype.label = {
    windowName: '子フォルダを操作',
    action: {
      group: "アクション",
      rename: "リネーム",
  ...
  function OperateChildFolders(windowName) {
    this.onClickedOK = bind(this.onClickedOK, this);
    var that;
    that = this;
    OperateChildFolders.__super__.constructor.call(this, this.label.windowName);
    this.addRadioButtonWithLabel({
      groupName: this.label.action.group,
      values: {
        rename: {
          text: this.label.action.rename,
          value: true,
          onClick: function() {
            return that.toggleInputTextEnabled(true);
          }
        },
        create: {
          text: this.label.action.create,
          onClick: function() {
            return that.toggleInputTextEnabled(true);
          }
        },
        sort: {
          text: this.label.action.sort,
          onClick: function() {
            return that.toggleInputTextEnabled(false);
          }
        }
      }
    });
    this.addInputTextWithLabel(this.label.arr, "");
    this.addInputTextWithLabel(this.label.prefix, "^-");
    this.addInputTextWithLabel(this.label.suffix, ".png");
    this.addRadioButtonWithLabel({
      groupName: this.label.sort.group,
      values: {
        ASC: {
          text: this.label.sort.ASC,
          value: true
        },
        DSC: {
          text: this.label.sort.DSC
        }
      }
    });
    this.addIntentButton(this.label.intent.ok, this.label.intent.cancel, 30);
  }

  OperateChildFolders.prototype.toggleInputTextEnabled = function(bool) {
    this.uDlg[this.label.arr].enabled = bool;
    this.uDlg[this.label.prefix].enabled = bool;
    return this.uDlg[this.label.suffix].enabled = bool;
  };

  OperateChildFolders.prototype.onClickedOK = function() {
    this.asc = this.uDlg[this.label.sort.group]["ASC"].value;
    if (this.uDlg[this.label.action.group]["rename"].value) {
      this.onSelectRename();
    }
    if (this.uDlg[this.label.action.group]["create"].value) {
      this.onSelectCreate();
    }
    if (this.uDlg[this.label.action.group]["sort"].value) {
      this.onSelectSort();
    }
    return this.uDlg.close();
  };

  OperateChildFolders.prototype.onSelectRename = function() {
    var arr, i, index, j, layer, layerSets, len, prefix, results, suffix;
    arr = this.getArrayFromInput(this.uDlg[this.label.arr].text);
    if (arr) {
      prefix = this.uDlg[this.label.prefix].text;
      suffix = this.uDlg[this.label.suffix].text;
      layerSets = activeDocument.activeLayer.layerSets;
      results = [];
      for (index = j = 0, len = layerSets.length; j < len; index = ++j) {
        layer = layerSets[index];
        i = this.asc ? index : layerSets.length - index - 1;
        if (layer.name && arr[i]) {
          results.push(layer.name = prefix + arr[i] + suffix);
        } else {
          results.push(void 0);
        }
      }
  ...
  OperateChildFolders.prototype.onSelectCreate = function() {
    var arr, i, index, j, k, layer, layerSets, len, len1, prefix, results, suffix;
    arr = this.getArrayFromInput(this.uDlg[this.label.arr].text);
    if (arr) {
      prefix = this.uDlg[this.label.prefix].text;
      suffix = this.uDlg[this.label.suffix].text;
      ...
      results = [];
      for (index = k = 0, len1 = layerSets.length; k < len1; index = ++k) {
        layer = layerSets[index];
        i = this.asc ? index : layerSets.length - index - 1;
        if (layer.name && arr[i]) {
          results.push(layer.name = prefix + arr[i] + suffix);
  ...
  OperateChildFolders.prototype.onSelectSort = function() {
    var i, index, j, k, l, layer, layerSets, len, len1, nextLayer, results, results1;
    layerSets = activeDocument.activeLayer.layerSets;
    if (this.asc) {
      results = [];
      for (i = j = 0, len = layerSets.length; j < len; i = ++j) {
        l = layerSets[i];
            ...
            if (index < layerSets.length - 1) {
              ...
              if (layer.name > nextLayer.name) {
                if (index === layerSets.length - 2) {
                  results1.push(nextLayer.moveBefore(layer));
                } else {
                  results1.push(layer.moveAfter(nextLayer));
                }
              } else {
      ...
      results1 = [];
      for (i = k = 0, len1 = layerSets.length; k < len1; i = ++k) {
        l = layerSets[i];
        results1.push((function() {
          var len2, m, results2;
          results2 = [];
          for (index = m = 0, len2 = layerSets.length; m < len2; index = ++m) {
            layer = layerSets[index];
            if (index < layerSets.length - 1) {
              nextLayer = layerSets[index + 1];
              if (layer.name < nextLayer.name) {
                if (index === layerSets.length - 2) {
                  results2.push(nextLayer.moveBefore(layer));
                } else {
                  results2.push(layer.moveAfter(nextLayer));
                }
              } else {
